...
-- Board utilities
type Board = [Int]
-- list of integer with we have left on each raws

initial :: Board
initial = [5,4,3,2,1]

finished :: Board ->Bool
finished b = all (==0) b -- here all is higher order function

valid :: Board -> Int -> Int -> Bool
valid b row num = b !! (row -1) >= num

move :: Board -> Int -> Int -> Board
-- this is where the list comprehense work.
...
main = do
    ...
    putStrLn "Board after removing 2 from row 1:"
    let nextBoard = move initial 4 2
    print nextBoard
    
    putStrLn "Is the game finished?"
    print (finished nextBoard)